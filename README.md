...
**Control goes to the guest** — `Ctrl-W`, `Ctrl-E` and `Ctrl-L` are how you
drive an Interlisp Exec or a Cedar viewer. In the browser the one exception
is `Ctrl-V`, which stays with the browser so clipboard paste keeps working,
and `Cmd` is left to the browser everywhere.
...
serialises keyboard and mouse into one message stream (HM Table 24) and the
microcode deposits it at the Alto's memory-mapped locations: **four 16-bit
words at `KBDAD` = `177034B`**, plus the mouse buttons and the 5-key keyset
at `177033B`. Everything is **active low** — a depressed key reads 0, an idle
key reads 1 — which matters more than it sounds: an unseeded cell reads as
*every key held down*, which is how Smalltalk-76 came to stop with "The
keyset is stuck".

Bit numbering is Xerox convention, **bit 0 = MSB**.

![Alto keyboard map](docs/images/alto-keyboard-map.svg)

The `word.bit` labels are Alto Hardware Manual Figure 6 (doc p.27); the
physical arrangement is reconstructed from the placement notes in Cedar's
`TerminalDefs.mesa` ("CTRL (large key left of A)", "LOCK (Alto lower left
corner)", `Spare1 "Look" (blank key right of BS)`) over the standard QWERTY
body, so it is a drawing rather than a traced photograph. Regenerate with
`tools/make_keyboard_svg.py docs/images/alto-keyboard-map.svg`; its table is
machine-checked against `dorado/src/display.c`'s `key_map`.
...
That is all 61 keys — "The Alto keyboard contains 61 keys" (HW Manual §5.1),
and the count is the cheapest check that nothing is missing. Alto I and
Alto II differ only in the keytops printed on the last few keys; the bits are
the same. **Look**, **Next** and **Swat** are the Alto's three unmarked keys
(`TerminalDefs.mesa` calls them `Spare1`, `Spare2`, `Spare3` and gives those
keytop names); Interlisp reads all three in `\eventkeys` as mouse-event
modifier bits, so they are not decorative.

Letters, digits, punctuation, Space, Return, Tab, Esc, Backspace, Delete,
Shift and Control map from the obvious host keys. The rest are stand-ins,
chosen for being free on a modern keyboard rather than for resembling the
keytops:
...
The mouse and keyset share `177033B`, also active low: **bit 13 Red = left**,
**bit 14 Blue = right**, **bit 15 Yellow = middle** (`TerminalDefs.mesa
KeyName`). See Controls above for the modifier chords.

Two per-world notes worth knowing:

- **Interlisp-D** puts its menus on the **right/blue** button. Press on the
  background for the system menu, move onto an item to select it, and move
  *right* out of an item marked `>` to open its submenu.
- **Cedar** reads the same words through `KeyBits` at `177033B` — that block
  starts at the mouse word and runs forward over the four keyboard words.

The audit behind this table — what was checked against which source, and the
input bugs it turned up — is `docs/parc-feedback-todo.md`, section A.
