...
| ★ **The Structure of Cedar** (Swinehart, Zellweger, Hagmann; ACM SIGPLAN 1985, pp. 230-244) | 15 pp. | `https://worrydream.com/refs/Swinehart_1985_-_The_Structure_of_Cedar.pdf`. The architects' own account, one version before our 6.1. **Scanned images -- text extraction returns binary noise; read the pages visually.** Figure 1 is the whole system on one page (Cedar Machine / Nucleus / Life Support / Applications). Settles four things we needed: colour is an option on a second frame buffer reached through Imager, and **Griffin** is the colour application; **FS will not open a remotely-named file for writing**, so served files are read-only caches and edits must be written locally first; a **DF file is the right container for "the sections and figures of a paper"**; and **Rollback "has become the conventional way to restart Cedar"**. See `docs/color-graphics-todo.md` and `docs/parc-feedback-todo.md` section M. |
...
| [lisp-distributions.md](lisp-distributions.md) | The per-release Interlisp-D distributions rebuilt from the 1108 floppies, and which ones boot. Intermezzo 1985 boots (checkpointed, deployed); Carol/Fugue/Harmony load and stop in Raid. Includes the sysout<->microcode version rules, the `.eb` stamp table, and TWO retracted conclusions with the tells that should have caught them. |
| [rebuilding-lisp-microcode.md](rebuilding-lisp-microcode.md) | ★ **How to rebuild a Dorado Lisp microcode from archive `.MB` source, byte-for-byte.** `mb2eb --loadrampage --stamp RAM,BCPL,LISP --name N in.MB out.eb 01070` reproduces a shipped `.EB` exactly bar the creation date. The three traps: the version stamp is a SEPARATE tool (`StampVersions`) so it is absent from any `.MB`; item order is IMAGE order; and the Lisp start address is **01070**, not the 01076 default -- a wrong entry point loads the whole sysout and silently never starts Lisp. Carol's and Fugue.6's Feb-1984 microcode exists only as source and is reconstructable this way. |
| [verilog-handoff.md](verilog-handoff.md) | ★ **Pick up the Verilog work from cold.** Task A is the two remaining Ethernet PROMs (self-contained); Task B is wiring the machine together and testing it against the C emulator, in four steps that land separately. Carries the backplane finding (no schematic needed -- 964 net names shared across boards' Term100 connectors), the gotchas that cost time, and the one open licence question. |
| [cedar-checkpoint.md](cedar-checkpoint.md) | ★ **Cedar's own Checkpoint / Rollback — the reported crash, FIXED (2026-08-14).** Two bugs in the POLLED germ IOCB arm, which until then only boot chains had exercised and boot chains only read: it ignored the command's Action fields (so the outload's writes were serviced as reads), and a file created at RUNTIME carries Pilot's CHS DiskAddresses even on a volume whose stored links are flat. Carries the `DORADO_MP_TRACE` instrument (Pilot's maintenance-panel codes decoded out of the cursor bitmap at `LONG[431B]`), the five theories eliminated by measurement, §10 on the browser (Save disk / Load disk), and three measurement traps — above all **"the end of the run is not the end of the operation"**: the restore repaints 2.2 B cycles after the inload finishes, so a short budget looks exactly like a failure. Gate: `make cedar-checkpoint-repro`. |
| [xerox-media-catalog.md](xerox-media-catalog.md) | What Xerox media survives in the archive and in what format — floppies, packs, sysouts, microcode — and which releases each covers. The map to consult before hunting for an artifact. |
| [lisp-release-gap-report.md](lisp-release-gap-report.md) | Per-release Interlisp-D gap audit: what is present, what is missing, and what is reconstructable from source. |
| [medley-library-access.md](medley-library-access.md) | Reaching the Medley library from the emulated worlds. |
| [smalltalk80-bootstrap.md](smalltalk80-bootstrap.md) | The Smalltalk-80 bootstrap track, including the `-s` (concatenate) vs `-l` (merge) distinction in `mb2eb` and the two-segment `DoradoInitialSmalltalk.eb` built exactly as Xerox's `DSemuRelease.cm` did. |
| [darkstar-extraction-plan.md](darkstar-extraction-plan.md) | Using the Darkstar Dandelion emulator as a reader for the Xerox Pilot 8-inch release floppies (`D/IO/FloppyDisk.cs`), to recover media our own tools cannot yet parse. |
| [smalltalk80-mailing-list-query.md](smalltalk80-mailing-list-query.md) | Drafted query to the Smalltalk community for the missing build-6 emulator / `d80ram.BR` microcode. |
