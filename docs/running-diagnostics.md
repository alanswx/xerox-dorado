...
| eventCounters  | genIO + setup-fault + **eventTrue** FIXED; now fails at `EVENTAHOLDERRLO` @4.1M (eventHold) | **Three fixes landed:** (1) genIO loopback (19d5226). (2) The `KFAULT3` setup fault was a junk-task bug — `IFUTest←B(0)` (IFU setup) spuriously *enabled* the periodic junk timer, waking the still-parked junk task (empty IM 0o7777) → halt; per HM §8.3 `IFUTest.15` only *disables* the junk request when set, never enables (ca44d3e). (3) HM §4.11 **event counters** — per-cycle EventCntA/B counting with the event select decoded from the MOS load and **tasksAll** gating (default counts only EmuOrFT = task 0 + 0o17; without it the interleaved sim/junk cycles inflate the count) — **eventTrue passes** (9bb3cc0). **Remaining:** `eventHold` counts Hold events but our counter sees 0. Traced precisely: the eventHold inner loop is two instructions (real 0o26↔0o23) — 0o26 is the `Fetch` (confirmed: kind=FETCH, disHold=0, latency=3, sets `task_md_ready`=cyc+3). The Md read (`rscr_(Md) or rscr` in source) lands on 0o23 (asel=4, bsel=1, **lc=6**) — but our `uinstr_reads_md` only flags asel=2 / bsel=0 / lc∈[2,5], so it doesn't recognize this instruction as consuming Md, `md_not_ready` never asserts, and no Hold fires. **Fix:** determine which of {asel=3/4, lc=6} actually reads Md here (HM Tables 8/10 + the shifter-Md path) and extend `uinstr_reads_md`. The Hold-signal-vs-disHold plumbing (4274713) is already correct; this is the last piece for eventHold. Then eventRef/eventIfuJump/eventMiss/eventEmuOrFT each need their event signal wired (EVC_EV_* flags defined). | small (extend uinstr_reads_md), then per-subtest wiring |
