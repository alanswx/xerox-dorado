...
| eventCounters  | genIO + setup-fault + **eventTrue** FIXED; now fails at `EVENTAHOLDERRLO` @4.1M (eventHold) | **Three fixes landed:** (1) genIO loopback (19d5226). (2) The `KFAULT3` setup fault was a junk-task bug — `IFUTest←B(0)` (IFU setup) spuriously *enabled* the periodic junk timer, waking the still-parked junk task (empty IM 0o7777) → halt; per HM §8.3 `IFUTest.15` only *disables* the junk request when set, never enables (ca44d3e). (3) HM §4.11 **event counters** — per-cycle EventCntA/B counting with the event select decoded from the MOS load and **tasksAll** gating (default counts only EmuOrFT = task 0 + 0o17; without it the interleaved sim/junk cycles inflate the count) — **eventTrue passes** (9bb3cc0). **Remaining:** `eventHold` counts Hold events, but our Md-Hold is gated off by `mcr.disHold` (which is set when the eventHold Fetch→Md loop runs), so 0 holds are counted. Either `iMem`/the setup must clear disHold for the test, or the 1-cycle MDI hold shouldn't be disHold-gated (risky — boot/games rely on disHold suppressing holds). Then eventRef/eventIfuJump/eventMiss/eventEmuOrFT each need their event signal wired (EVC_EV_* flags already defined). | medium (per-subtest event wiring) |
