...
| memA           | TIMEOUT (task-0 starved by sim tasks) | TASKSIM now done; root cause refined — memA's **memory simulator** (`memSimInit`, `memSubrsA.mc`) runs in a sim task that fetch/stores a roving pointer "that always hold" + a delay loop, *relying on the cache-miss memory HOLD to pace itself* (its own comment: "time the memory task spends being **held by its fetches**"). Without memory HOLD the sim never idles, so the sim tasks (2 + 0o12) ping-pong ~6400×/500k steps and **starve task 0** (the actual test; `ready=0005` but never scheduled). **Needs the memory cache-miss HOLD (the real gap B1)** + HOLDSIM injection. The existing `DORADO_HOLD` (Md-not-ready stall) alone does not fix it. | LARGE (memory-Hold project) |
