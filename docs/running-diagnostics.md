...
| eventCounters  | genIO FIXED (was FAIL @971 `GENIOERR2`); now HALTs @11206 (`KFAULT3` fault-trap loop at pc 0) | **genIO loopback DONE** (commit 19d5226): the IFU-board plug ties GenOut→GenIn, so `GenIn'`(EventCntA', FF=0o171) reads back `GenOut`(EventCntB, FF=0o131) in GenIO mode (set by `mos←useGenIO`=0 when both counter enables clear). **Next blocker is NOT event counting** — confirmed by instrumenting `evc_a_enable` (it never sets), the run faults into `KFAULT3` during the post-genIO setup (`initJunkForWakeups`/`iMem`, arms the IFU via PCF←B) *before* any counter test runs. A trial event-counter implementation (per-cycle EventCntA/B tick on True/Hold/Ref/IfuJump/Miss, grounded in eventCounters1/2.mc — decode A enable=B[4]/event=B[5:7], B enable=B[5]/event=B[13:15]) was written but is unexercised until the KFAULT3 setup fault is fixed, so it was reverted. Investigate the fault first (memory or IFU fault in the setup), THEN add+validate event counting. | medium (fault in setup, then event counting) |
