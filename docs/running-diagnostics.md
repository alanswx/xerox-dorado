...
| memA           | FAIL @87k (`CATUPADDRERR`), was 3M TIMEOUT | Starvation FIXED (commit 40cc535). It was NOT a Hold gap: our junk timer woke task 2 (TPC=0, no handler in memA) which ran garbage, never blocked, and starved task 0. Cause: IFUReset spuriously enabled the junk task in every world; AEmu starts it explicitly (`Wakeup[JNK]`+`AckJunkTW`), memA never does. Now memA runs the real memory tests in task 0 and fails at `CATUPADDRERR` (0o4232) — a **cache-address-section** up-count test (memSubrsD's cache-address machinery: dVA←Victim / CFlags / cache-address counting). Next: model that, then likely more cache/Pipe + HOLDSIM. | cache-address model + Hold |
