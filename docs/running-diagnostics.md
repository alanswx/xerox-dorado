...
| eventCounters  | genIO + setup-fault + **eventTrue** FIXED; now fails at `EVENTAHOLDERRLO` @4.1M (eventHold) | **Three fixes landed:** (1) genIO loopback (19d5226). (2) The `KFAULT3` setup fault was a junk-task bug — `IFUTest←B(0)` (IFU setup) spuriously *enabled* the periodic junk timer, waking the still-parked junk task (empty IM 0o7777) → halt; per HM §8.3 `IFUTest.15` only *disables* the junk request when set, never enables (ca44d3e). (3) HM §4.11 **event counters** — per-cycle EventCntA/B counting with the event select decoded from the MOS load and **tasksAll** gating (default counts only EmuOrFT = task 0 + 0o17; without it the interleaved sim/junk cycles inflate the count) — **eventTrue passes** (9bb3cc0). **Remaining:** `eventHold` counts Hold events but our counter sees 0. Traced precisely (and *correcting* an earlier guess): the eventHold inner loop in **this .mb (UnBug.bfs) is `Fetch`(real 0o26, asel=3=Fetch←T) ↔ an RM op (real 0o23, asel=4=A←RM/STK, bsel=1=B←RM/STK, lc=6=RM/STK←Pd)** — it reads **no Md at all** (verified against HM Tables 8/10), so our Md-read Hold never fires. The `eventCounters2.mc` source I mirrored (from eventCountersSources.dm!5_) shows `rscr_(Md) or rscr`, so the **UnBug.bfs .mb is a different build** whose loop reads no Md. **DIAGNOSIS CORRECTED (2026-06-25, this session): the eventHold hold source is the HM §3.12 HOLDSIM debug register, NOT a §5.4 reference/port-busy Hold.** Triple-confirmed by a per-cycle trace of the loop: (1) the 8-cycle loop is `tk=0 0o26`(Fetch←T) + `tk=0 0o23`(lc=6 RM/STK←Pd) — neither reads Md, so md_not_ready is never true; (2) the §3.12 **sim task (task 12)** runs `0o5202`=`Hold&TaskSim←B` (FF=0o154) every iteration, arming HOLDSIM=0o362 (~1.03M times) — the only possible hold source; (3) a single Fetch every 8 cycles can't cause a port-busy Hold either. So `eventHold` needs the **§3.12 HOLDSIM recirculating-shift-register hold injection** (postamble.mc: "8-bit recirculating shift register in which a 1 in bit 7 causes HOLD two instructions later"). **Implemented this session (non-regressing, all standing gates green):** the HOLDSIM recirculation (verified 0o362→0o171→0o274→0o136→0o362, popcount-preserving) + a 2-instruction-delay trigger feeding `EVC_EV_HOLD` (cpu.c task_schedule + the hold block; cpu.h `holdsim`/`holdsim_delay`/`holdsim_hold`). **HM §3.12 (transcoded HM p.25-26) pins the exact semantics:** HOLDSIM "recirculating shift register in which a 1 in bit 7 causes HOLD two instructions later"; "its clocks are **not disabled by HOLD**" (so the register keeps shifting through a hold — HOLD of infinite duration is impossible); `←200₈` (one 1-bit) → HOLD every 8th cycle. So the .mb's HOLDSIM=0o362 (five 1-bits) legitimately yields multiple holds per 8 cycles — the **.mb expects HOLDSIM's native count, NOT the source build's "1 per reference"** (the `rscr_(Md)` source is a different build). **Landed this session:** a **HOLD freeze** path — a §3.12-injected hold freezes the engine and stays on the current task (the per-cycle ticks, incl. the HOLDSIM shift, still run; HM "clocks not disabled by HOLD") rather than jump-to-self/switch. This moves the failure 4.1M→5.0M steps (several outer iterations now pass) and is non-regressing (make test digest unchanged, kernel PASS@3765457, memA PASS — its step count shifts 220.8M→223.1M because memA's own SETHOLD now injects holds, still reaching DONE). **Remaining blocker — ISOLATED (2026-06-26).** eventHold-A expects exactly **64 holds = rscr2+1 = 1 per reference** (read directly: `CTRELAPSEDLO`=elapsed vs `RSCR`=0o100=expected; the source build's "1 per reference" DOES apply to the .mb — an earlier note that the .mb expects HOLDSIM's native multi-hold count was wrong; the 64 exactly-matching iterations seen earlier were **eventTrue**, not eventHold). The model counts only **1**. Per-cycle trace of the inner loop (10-cycle period: `tk0 0o23`, 4 sim, `tk0 0o26`=Fetch, 4 sim) shows the §3.12 HOLDSIM hold **always lands on the sim task's `0o5202`=`Hold&TaskSim←B` arm cycle (frozen there, re-running), NEVER on a task-0 reference** — so the EmuOrFT-gated counter never ticks. Root cause: the hold is **phase-locked to the HOLDSIM reload** — the sim task reloads HOLDSIM=0o362 at `0o5202` every ~5 cycles (2× per 10-cycle reference), and the hold fires at a fixed offset that lands back on the next `0o5202`. Fixing it needs the hold to land on the task-0 reference once per reference. **Progress (commit a18a8ba): 5.0M→7.8M steps, hold count 1→4** via two grounded, non-regressing fixes — (1) HOLDSIM recirculates **left** not right (HM §3.12 example `←0o200` → HOLD on the 4th instruction; left-shift reproduces it, right gives instr 10); (2) the event-counter **True** event counts only NON-held cycles (a HOLD stretches an instruction; otherwise held cycles inflate eventTrue past its 0o20 slop — which is why a bare left-shift regressed eventTrue). **Still failing:** only 4 of 64 references get a counted hold — the §3.12 holds do not lock 1:1 to the references because the **sim task dominates the schedule** (task-0 runs ~1 cycle in 8; its 3-instruction activation + the holds it absorbs leave references sparse), so most holds land on sim (0o5202) cycles. Closing it requires the **sim-task (TASKSIM) scheduling cycle-exact** so a hold lands on each 0o26 — the holistic cycle-accurate sim-scheduling work, not reachable by incremental tuning. No-slop oracle, zero game/Cedar regression risk. | large (cycle-exact §3.12 sim-task scheduling so holds lock 1:1 to references) |
