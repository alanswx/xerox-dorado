...
| eventCounters  | genIO FIXED (was FAIL @971 `GENIOERR2`); now HALTs @11206 in the real counter tests | **genIO loopback DONE** (commit 19d5226): the IFU-board plug ties GenOut→GenIn, so `GenIn'`(EventCntA', FF=0o171) reads back `GenOut`(EventCntB, FF=0o131) in GenIO mode (set by `mos←useGenIO`=0 when both counter enables clear). Remaining: real **per-cycle event counting** (HM §4.11/§12.3) — the counter tests need EventCntA/B to actually tick on programmed events; currently unmodeled, so a counter-driven dispatch jumps to an invalid PC. | medium (event counting) |
