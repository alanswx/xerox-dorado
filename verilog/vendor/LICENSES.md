...
**DECISION (2026-08-15): keep it.** It works, it is the only Verilator-ready
6532 to hand, and the project is not shipping commercially. This note is the
record, so the constraint is known rather than discovered later.
...
here impose. If that ever matters, three ways out:
...
3. **Port our own** -- the preferred route if the licence ever binds.
   `dorado/src/baseboard.c` already models the 6532 well
