// VENDORED, WITH A MINIMAL PATCH.
//
// Source: MiSTer-devel/Atari7800_MiSTer rtl/RIOT/M6532.sv (k7800 by Jamie
// Blanks). The Atari 7800 core's RIOT is SystemVerilog; the 2600 core's is
// VHDL, which Verilator cannot take -- hence this one.
//
// *** LICENCE: CC BY-NC 4.0 -- NONCOMMERCIAL. ***
// That is more restrictive than the rest of this repository. See
// verilog/vendor/LICENSES.md before shipping anything that includes it.
//
// PATCH: the output ports assigned inside always blocks are declared
// `output logic` rather than `output wire`. Quartus infers that; Verilator
// follows IEEE 1800 strictly and rejects a procedural assignment to a wire
// (PROCASSWIRE). No behavioural change.
...
(
	...
	output logic [7:0] PA_out,    // NOTE that port output must be fed back to input
	input  [7:0] PB_in,     // if not altered by a peripheral, in order for
	output logic [7:0] PB_out,    // the chip to read properly!
	output       logic       oe         // Output enabled (always 8 bits)
);
